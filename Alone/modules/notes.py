from Alone import app, BOT_USERNAME, COMMAND_HANDLER
from pyrogram import filters
from Alone.database.notes_db import *
from Alone.helpers.notes_func import GetNoteMessage, exceNoteMessageSender, privateNote_and_admin_checker
from Alone.helpers.status import user_admin
...
async def PrivateNote(client, message):
    ...
    if len(message.command) >= 2:
        ...
        if await is_pnote_on(chat_id):
            await message.reply(
                "Your notes are currently being sent in private. Alone will send a small note with a button which redirects to a private chat.",
                quote=True
            )
        else:
            await message.reply(
