...
from Alone import app, LOG_CHANNEL_ID
...
`@{app.me.username} Hi @ALONE_WAS_BOT`
`@{app.me.username} Hi @all`

"""
   key = InlineKeyboardMarkup (
     [[
       InlineKeyboardButton ("TRY NOW", switch_inline_query='Hi @ALONE_WAS_BOT') ]]
   )
    ...
    await query.answer(
        results=[
            InlineQueryResultPhoto(
                ...
                photo_url='https://t.me/ALONE_WAS_BOT',
                description=f'@{app.me.username} Hello @KIRITO120',
                reply_markup=InlineKeyboardMarkup ([[InlineKeyboardButton ("🔗", url='t.me/AloneXBots')]]),
...
async def del_whisper(app,query):
   ...
   if not query.from_user.id == user:
     ...
     reply_markup = InlineKeyboardMarkup(
      [[
        InlineKeyboardButton("DEV. 🔗", url="https://t.me/ALONE_WAS_BOT")
      ]]
    )
